...
length(unique(genus_occurrence_edi$Genus)) #960 genera
...
sites_map2<-sites_map+geom_path(color="black",aes(group=group))+geom_point(data = genus.occurrence.all, aes(x = Longitude_transformed, y = Latitude_transformed, color = provider2, shape=provider2), alpha = 1, size=0.8)+
  ...
  theme_bw()+theme(axis.ticks.y = element_blank(), axis.ticks.x = element_blank(), axis.text.x=element_blank(), axis.text.y=element_blank(), axis.title.x=element_blank(), axis.title.y=element_blank(), 
                   panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
                   panel.border = element_rect(colour = "black", size=2), legend.position="top", legend.text = element_text(size = 25), legend.title = element_blank())
sites_map3<-sites_map2+ggsn::scalebar(states, dist = 500, dist_unit="km",st.size=5, height=0.01,transform =FALSE, location="bottomleft")+ggsn::north(states, scale=0.1,symbol=1,location="bottomright")

sites_map3
ggsave("~/Documents/WaterCube/Ch.3/aquatic_insects/Figures/all_sites2.pdf", sites_map3, height =8, width = 12, dpi = 300)
ggsave("~/Documents/WaterCube/Ch.3/aquatic_insects/Figures/all_sites2.png",sites_map3, height = 8, width = 12, dpi = 300)
